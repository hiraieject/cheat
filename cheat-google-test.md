...
| 引数名                            | 説明                                                     | 例                                              |
|:----------------------------------|:---------------------------------------------------------|:------------------------------------------------|
| `--gtest_filter`                  | テストケースまたはテストをフィルタリングして実行します。 | `$ test_executable --gtest_filter=Test_Case1.*` |
| `--gtest_also_run_disabled_tests` | 無効にされたテストも実行します。                         |                                                 |
| `--gtest_repeat`                  | テストを指定回数繰り返し実行します。                     | `$ test_executable --gtest_repeat=10`           |
| `--gtest_shuffle`                 | テストの実行順序をランダムにシャッフルします。           | `$ test_executable --gtest_shuffle`             |

### テストの出力とレポート

| 引数名               | 説明                                     | 例                                          |
|:---------------------|:-----------------------------------------|:--------------------------------------------|
| `--gtest_output`     | テスト結果の出力形式とパスを設定します。 | `$ test_executable --gtest_output="xml:./"` |
| `--gtest_list_tests` | 実行可能なテストの一覧を出力します。     |                                             |

### テストの動作

| 引数名                        | 説明                                       | 例                                                   |
|:------------------------------|:-------------------------------------------|:-----------------------------------------------------|
| `--gtest_break_on_failure`    | テスト失敗時に停止します。                 |                                                      |
| `--gtest_catch_exceptions`    | 例外の捕捉を制御します（デフォルトはON）。 |                                                      |
| `--gtest_color`               | 色付けの制御します。                       | `$ test_executable --gtest_color=yes`                |
| `--gtest_death_test_style`    | 死亡テストのスタイルを設定します。         | `$ test_executable --gtest_death_test_style=fast`    |
| `--gtest_death_test_use_fork` | 死亡テストにforkを使用するか設定します。   | `$ test_executable --gtest_death_test_use_fork=true` |

### その他

| 引数名                     | 説明                                              | 例                                                           |
|:---------------------------|:--------------------------------------------------|:-------------------------------------------------------------|
| `--gtest_print_time`       | テスト実行時間の出力を制御します。                |                                                              |
| `--gtest_random_seed`      | シャッフルのためのシード値を設定します。          | `$ test_executable --gtest_random_seed=12345`                |
| `--gtest_stream_result`    | テスト結果を指定されたURIにストリーミングします。 | `$ test_executable --gtest_stream_result=http://example.com` |
| `--gtest_throw_on_failure` | アサーション失敗時に例外を投げるか制御します。    | `$ test_executable --gtest_throw_on_failure`                 |

これらのオプションは自由に組み合わせて使用できます。
