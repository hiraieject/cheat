...
- --gtest_output: テスト結果の出力形式とパスを設定します。
...
- --gtest_color: 色付けの制御します。
- --gtest_death_test_style: 死亡テストのスタイルを設定します。
- --gtest_death_test_use_fork: 死亡テストにforkを使用するか設定します。

### その他

- --gtest_print_time: テスト実行時間の出力を制御します。
- --gtest_random_seed: シャッフルのためのシード値を設定します。
- --gtest_stream_result: テスト結果を指定されたURIにストリーミングします。
- --gtest_throw_on_failure: アサーション失敗時に例外を投げるか制御します。
